...
@defcomp DamageAggregator_BresslerMortality begin
    ...
    include_bressler_mortality = Parameter{Bool}(default=true)
    ...
    damage_bressler_mortality = Parameter(index=[time,country], unit="US\$2005/yr")
    ...
    bressler_mortality_damage       = Variable(index=[time], unit="US\$2005/yr")
    ...
    bressler_mortality_damage_domestic        = Variable(index=[time], unit="US\$2005/yr")

    capped                                 = Variable(index=[time])
    ...
    function run_timestep(p, v, d, t)
        ...
        v.bressler_mortality_damage[t]      = sum(p.damage_bressler_mortality[t,:]) 

        # cap total damages at gdp
        gdp = sum(p.gdp[t,:]) * 1e9
        ...
        v.total_damage[t] =
            ...
            (p.include_bressler_mortality   ? v.bressler_mortality_damage[t] : 0.) +
        ...
        v.bressler_mortality_damage_domestic[t]         = sum(p.damage_bressler_mortality[t, v.domestic_idxs_country_dim_int])
        ...
        v.total_damage_domestic[t] = 
            ...
            (p.include_bressler_mortality   ? v.bressler_mortality_damage_domestic[t] : 0.)
    end
end