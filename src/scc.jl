using Dates, CSVFiles, DataFrames, MimiGIVE, Mimi, Query
"""
...
"""
function compute_scc(m::Model = get_modified_model(); 
            year::Union{Int, Nothing} = nothing, 
            last_year::Int = MimiGIVE._model_years[end], 
            prtp::Union{Float64,Nothing} = 0.015, 
            eta::Union{Float64,Nothing} = 1.45,
            discount_rates = nothing,
            discount_factor::Symbol = :gdp,
            certainty_equivalent = false,
            fair_parameter_set::Symbol = :random,
            fair_parameter_set_ids::Union{Vector{Int}, Nothing} = nothing,
            rffsp_sampling::Symbol = :random,
...
function _compute_scc(mm::MarginalModel; 
    ...
    if domestic
        main_marginal_damages = mm[:DamageAggregator, :total_damage_domestic] .* MimiGIVE.scc_gas_molecular_conversions[gas] 
        ciam_marginal_damages = mm.base[:DamageAggregator, :include_slr] ? all_ciam_marginal_damages.domestic : fill(0., length(MimiGIVE._model_years)) 
    else
        main_marginal_damages = mm[:DamageAggregator, :total_damage] .* MimiGIVE.scc_gas_molecular_conversions[gas] 
        ciam_marginal_damages = mm.base[:DamageAggregator, :include_slr] ? all_ciam_marginal_damages.globe : fill(0., length(MimiGIVE._model_years)) 
    end
    ...
    global_pc_gdp = mm.base[:PerCapitaGDP, :global_pc_gdp]
    ...
    if discount_rates!==nothing
        ...
        for dr in discount_rates
            ...
            if isnothing(dr.ew) # no equity weighting
                ...
                elseif discount_factor == :gdp
                    df = [((global_pc_gdp[year_index]/global_pc_gdp[i])^dr.eta * 1/(1+dr.prtp)^(t-year) for (i,t) in enumerate(MimiGIVE._model_years) if year<=t<=last_year)...]
                    scc = sum(df .* marginal_damages[year_index:last_year_index])
                end     

            elseif dr.ew==:gdp # equity weight using gdp per capita
                
                ag_marginal_damages     = mm[:Agriculture, :agcost] .* MimiGIVE.scc_gas_molecular_conversions[gas] * 1e9 # fund regions
                en_marginal_damages     = mm[:energy_damages, :energy_costs_dollar] .* MimiGIVE.scc_gas_molecular_conversions[gas] * 1e9 # country
                cromar_health_marginal_damages = mm[:CromarMortality, :mortality_costs] .* MimiGIVE.scc_gas_molecular_conversions[gas] # country
                bressler_health_marginal_damages = mm[:BresslerMortality, :mortality_costs_net] .* MimiGIVE.scc_gas_molecular_conversions[gas] # country
                ...
                cromar_health_scc_in_utils = sum(
                    cromar_health_marginal_damages[i,r] / pc_gdp_for_health[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                    for (i,t) in enumerate(MimiGIVE._model_years), r in 1:n_regions_for_health if year<=t<=last_year
                )

                bressler_health_scc_in_utils = sum(
                    bressler_health_marginal_damages[i,r] / pc_gdp_for_health[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                    for (i,t) in enumerate(MimiGIVE._model_years), r in 1:n_regions_for_health if year<=t<=last_year
                )
                ...
                ag_scc_in_utils = sum(
                    ...
                    for (i,t) in enumerate(MimiGIVE._model_years), r in 1:n_regions_for_ag if year<=t<=last_year
                )
                ...
                en_scc_in_utils = sum(
                    ...
                    for (i,t) in enumerate(MimiGIVE._model_years), r in 1:n_regions_for_en if year<=t<=last_year
                )
                ...
                slr_marginal_damages = mm.base[:DamageAggregator, :include_slr] ? all_ciam_marginal_damages.country : fill(0., length(MimiGIVE._model_years), n_regions_for_slr) # 145 countries (coastal only), only run ciam if needed
                slr_scc_in_utils = sum(
                    slr_marginal_damages[i,r] / pc_gdp_for_slr[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                    for (i,t) in enumerate(MimiGIVE._model_years), r in 1:n_regions_for_slr if year<=t<=last_year
                )

                # sum up total utils for included sectors to calculate scc
                total_utils = 
                    (mm.base[:DamageAggregator, :include_cromar_mortality]      ? cromar_health_scc_in_utils : 0.) +
                    (mm.base[:DamageAggregator, :include_bressler_mortality]    ? bressler_health_scc_in_utils : 0.) +
                ...
                if dr.ew_norm_region == "globe"
                    scc = mm.base[:PerCapitaGDP, :global_pc_gdp][year_index]^dr.eta * total_utils
                else
            ...
            elseif dr.ew==:consumption # equity weight using gdp per capita net of climate damages

                non_slr_marginal_damages = mm[:DamageAggregator, :total_damage_regions] .* MimiGIVE.scc_gas_molecular_conversions[gas] # fund regions
                ...
                non_slr_scc_in_utils = sum(
                    ...
                    for (i,t) in enumerate(MimiGIVE._model_years), r in 1:n_regions if year<=t<=last_year
                )
                ...
                if dr.ew_norm_region == "globe"
                    scc = mm.base[:global_netconsumption, :net_cpc][year_index]^dr.eta * total_utils
                else
...
function modified_post_trial_func(mcs::SimulationInstance, trialnum::Int, ntimesteps::Int, tup)
    ...
    scc_values, intermediate_ce_scc_values, md_values, cpc_values, slr_damages, year, last_year, discount_rates, discount_factor, gas, ciam_base, ciam_modified, segment_fingerprints, options = Mimi.payload2(mcs)
    ...
    if options.compute_sectoral_values
        ...
        bressler_mortality_mds = post_trial_mm[:DamageAggregator, :bressler_mortality_damage]
        ...
        if options.compute_domestic_values
            ...
            bressler_mortality_mds_domestic = post_trial_mm[:DamageAggregator, :bressler_mortality_damage_domestic]
    ...
    if options.save_md
        ...
        if options.compute_sectoral_values
            ...
            md_values[(region=:globe, sector=:bressler_mortality)][trialnum, :] = bressler_mortality_mds[MimiGIVE._damages_idxs]
        ...
        if options.compute_domestic_values
            ...
            if options.compute_sectoral_values
                ...
                md_values[(region=:domestic, sector=:bressler_mortality)][trialnum, :]   = bressler_mortality_mds_domestic[MimiGIVE._damages_idxs]
    ...
    global_pc_gdp = base[:PerCapitaGDP, :global_pc_gdp]
    ...
    for dr in discount_rates
        ...
        if isnothing(dr.ew) # no equity weighting
            ...
            elseif discount_factor == :gdp
                df = [((global_pc_gdp[year_index]/global_pc_gdp[i])^dr.eta * 1/(1+dr.prtp)^(t-year) for (i,t) in enumerate(MimiGIVE._model_years) if year<=t<=last_year)...]
                if options.certainty_equivalent
                    df_ce = [((1. / global_pc_gdp[i])^dr.eta * 1/(1+dr.prtp)^(t-year) for (i,t) in enumerate(MimiGIVE._model_years) if year<=t<=last_year)...] # only used if optionas.certainty_equivalent=true
                end
            end
            ...
            if options.compute_sectoral_values
                ...
                scc = sum(df .* bressler_mortality_mds[year_index:last_year_index])
                scc_values[(region=:globe, sector=:bressler_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                ...
                if options.certainty_equivalent
                    ...
                    intermediate_ce_scc = sum(df_ce .* bressler_mortality_mds[year_index:last_year_index])
                    intermediate_ce_scc_values[(region=:globe, sector=:bressler_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = intermediate_ce_scc
                ...
                if options.compute_domestic_values
                    ...
                    scc = sum(df .* bressler_mortality_mds_domestic[year_index:last_year_index])
                    scc_values[(region=:domestic, sector= :bressler_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                    ...
                    if options.certainty_equivalent
                        ...
                        intermediate_ce_scc = sum(df_ce .* bressler_mortality_mds_domestic[year_index:last_year_index])
                        intermediate_ce_scc_values[(region=:domestic, sector= :bressler_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = intermediate_ce_scc
        ...
        elseif dr.ew==:gdp # equity weight with gdp
            ...
            cromar_health_marginal_damages = post_trial_mm[:DamageAggregator, :damage_cromar_mortality] # country
            bressler_health_marginal_damages = post_trial_mm[:DamageAggregator, :damage_bressler_mortality] # country
            # note slr_marginal_damages allocated below for conciseness of variables
            ...
            pc_gdp_for_slr = [fill(0., 2020-1750, 145); repeat(ciam_base[:slrcost, :ypcc][1:end-1,:], inner=(10,1)); ciam_base[:slrcost, :ypcc][end:end,:]]
            n_regions_for_slr = size(pc_gdp_for_slr, 2)
            slr_marginal_damages =  post_trial_mm.base[:DamageAggregator, :include_slr] ? ciam_mds.country : fill(0., length(MimiGIVE._model_years), n_regions_for_slr) # 145 countries (coastal only), only run ciam if included
            slr_scc_in_utils = sum(
                    slr_marginal_damages[i,r] / pc_gdp_for_slr[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                    for (i,t) in enumerate(MimiGIVE._model_years), r in 1:n_regions_for_slr if year<=t<=last_year
                )
            ...
            # sum up total utils for included sectors to calculate scc
            total_utils =
                (base[:DamageAggregator, :include_cromar_mortality]         ? cromar_health_scc_in_utils : 0.) +
                (base[:DamageAggregator, :include_bressler_mortality]       ? bressler_health_scc_in_utils : 0.) +
                ...
                if dr.ew_norm_region == "globe"
                    scc = base[:PerCapitaGDP, :global_pc_gdp][year_index]^dr.eta * total_utils
                    scc_values[(region=:globe, sector=:total, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                else  
            ...
            if options.compute_sectoral_values
                if dr.ew_norm_region == "globe"
                    scc = base[:PerCapitaGDP, :global_pc_gdp][year_index]^dr.eta * cromar_health_scc_in_utils
                    scc_values[(region=:globe, sector=:cromar_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc

                    scc = base[:PerCapitaGDP, :global_pc_gdp][year_index]^dr.eta * bressler_health_scc_in_utils
                    scc_values[(region=:globe, sector=:bressler_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc

                    scc = base[:PerCapitaGDP, :global_pc_gdp][year_index]^dr.eta * ag_scc_in_utils
                    scc_values[(region=:globe, sector=:agriculture, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                    
                    scc = base[:PerCapitaGDP, :global_pc_gdp][year_index]^dr.eta * en_scc_in_utils
                    scc_values[(region=:globe, sector=:energy, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc

                    scc = base[:PerCapitaGDP, :global_pc_gdp][year_index]^dr.eta * slr_scc_in_utils
                    scc_values[(region=:globe, sector=:slr, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                else  
                    scc = base[:PerCapitaGDP, :pc_gdp][year_index,normalization_region_index]^dr.eta * cromar_health_scc_in_utils
                    scc_values[(region=:globe, sector=:cromar_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc

                    scc = base[:PerCapitaGDP, :pc_gdp][year_index,normalization_region_index]^dr.eta * bressler_health_scc_in_utils
                    scc_values[(region=:globe, sector=:bressler_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
        ...
        elseif dr.ew==:consumption # equity weight with consumption
            ...
            cromar_health_marginal_damages = post_trial_mm[:DamageAggregator, :damage_cromar_mortality_regions] # fund regions
            bressler_health_marginal_damages = post_trial_mm[:DamageAggregator, :damage_bressler_mortality_regions] # fund regions
            ...
            cromar_health_scc_in_utils = sum(
                cromar_health_marginal_damages[i,r] / pc_consumption[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                for (i,t) in enumerate(MimiGIVE._model_years), r in 1:n_regions if year<=t<=last_year
            )

            bressler_health_scc_in_utils = sum(
                bressler_health_marginal_damages[i,r] / pc_consumption[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                for (i,t) in enumerate(MimiGIVE._model_years), r in 1:n_regions if year<=t<=last_year
            )
            ...
            if dr.ew_norm_region == "globe"
                scc = base[:global_netconsumption, :net_cpc][year_index]^dr.eta * total_utils
                scc_values[(region=:globe, sector=:total, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
            else
            ...
            if options.compute_sectoral_values
                ...
                scc = base[:regional_netconsumption, :net_cpc][year_index,normalization_region_index]^dr.eta * cromar_health_scc_in_utils
                scc_values[(region=:globe, sector=:cromar_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc

                scc = base[:regional_netconsumption, :net_cpc][year_index,normalization_region_index]^dr.eta * bressler_health_scc_in_utils
                scc_values[(region=:globe, sector=:bressler_mortality, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
...
function _compute_modified_scc_mcs(mm::MarginalModel, 
    ...
    sectors = compute_sectoral_values ? [:total,  :cromar_mortality, :bressler_mortality, :agriculture, :energy, :slr] : [:total]
    ...
    payload = [scc_values, intermediate_ce_scc_values, md_values, cpc_values, slr_damages, year, last_year, discount_rates, discount_factor, gas, ciam_base, ciam_modified, segment_fingerprints, options]
    ...
    # unpack the payload object
    scc_values, intermediate_ce_scc_values, md_values, cpc_values, slr_damages, year, last_year, discount_rates, discount_factor, gas, ciam_base, ciam_modified, segment_fingerprints, options = Mimi.payload2(sim_results)
...
function _compute_ciam_marginal_damages(base, modified, gas, ciam_base, ciam_modified, segment_fingerprints; CIAM_foresight, CIAM_GDPcap, pulse_size)
    MimiGIVE.update_ciam!(ciam_base, base, segment_fingerprints)
    MimiGIVE.update_ciam!(ciam_modified, modified, segment_fingerprints)
    ...
    if CIAM_foresight == :perfect
        OptimalCost_base = MimiGIVE.compute_PerfectForesight_OptimalCosts(ciam_base)
        OptimalCost_modified = MimiGIVE.compute_PerfectForesight_OptimalCosts(ciam_modified)
    ...
    OptimalCost_base_country = Array{Float64}(undef, length(MimiGIVE._damages_years), num_ciam_countries)
    OptimalCost_modified_country = Array{Float64}(undef, length(MimiGIVE._damages_years), num_ciam_countries)
    ...
    if CIAM_GDPcap
        ...
        base_lim_cnt = fill(0., length(MimiGIVE._damages_years), num_ciam_countries)
        modified_lim_cnt = fill(0., length(MimiGIVE._damages_years), num_ciam_countries)
    end
    ...
    damages_marginal_domestic = (damages_modified_domestic .- damages_base_domestic) .* MimiGIVE.scc_gas_molecular_conversions[gas] ./ (MimiGIVE.scc_gas_pulse_size_conversions[gas] .* pulse_size) # adjust for the (1) molecular mass and (2) pulse size
    ...
    damages_marginal = (damages_modified .- damages_base) .* MimiGIVE.scc_gas_molecular_conversions[gas] ./ (MimiGIVE.scc_gas_pulse_size_conversions[gas] .* pulse_size) # adjust for the (1) molecular mass and (2) pulse size
    ...
    damages_marginal_country = damages_marginal_country .* MimiGIVE.scc_gas_molecular_conversions[gas] ./ (MimiGIVE.scc_gas_pulse_size_conversions[gas] .* pulse_size) # adjust for the (1) molecular mass and (2) pulse size
    ...
    return (globe               = [fill(0., 2020 - MimiGIVE._model_years[1]); damages_marginal], # billion USD $2005
            domestic            = [fill(0., 2020 - MimiGIVE._model_years[1]); damages_marginal_domestic], # billion USD $2005
            country             = [fill(0., 2020 - MimiGIVE._model_years[1], 145); damages_marginal_country], # billion USD $2005
            damages_base        = [fill(0., 2020 - MimiGIVE._model_years[1]); damages_base], # billion USD $2005
            damages_modified    = [fill(0., 2020 - MimiGIVE._model_years[1]); damages_modified], # billion USD $2005
            damages_base_domestic = [fill(0., 2020 - MimiGIVE._model_years[1]); damages_base_domestic], # billion USD $2005
            damages_modified_domestic = [fill(0., 2020 - MimiGIVE._model_years[1]); damages_modified_domestic], # billion USD $2005
...
the default model from get_modified_model() is used as the base model.
"""
function get_marginal_model(m::Model; year::Union{Int, Nothing} = nothing, gas::Symbol, pulse_size::Float64)
    ...
    !(year in MimiGIVE._model_years) ? error("Cannot add marginal emissions in $year, year must be within the model's time index $MimiGIVE._model_years.") : nothing
    ...
    mm = create_marginal_model(m, MimiGIVE.scc_gas_pulse_size_conversions[gas] .* pulse_size)
